    ...
    public static function _updateForms($forms, $template) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (!$form_id || !$contact_form) {
                ...
                $form_title = "";
                if ($template === "footer") {
                    $form_title = sprintf(__('Form: %s', 'trailblazers'), "Footer");
                }
                if ($template === "header") {
                    $form_title = sprintf(__('Form: %s', 'trailblazers'), "Header");
                }
                if ($template === "custom") {
                    if (is_singular()) {
                        global $post;
                    ...
                    $form_title = sprintf(__('Form: %s', 'trailblazers'), "Custom-" . $id);
                }
                if ($count > 1) {
                    $form_title .= ' (' . ($i + 1) . ')';
                }
